    ...
    public class DCMEditorWindow : EditorWindow {
        ...
        void DrawAtlasedInformation() {
            ...
            for (int i = 0; i < _infoFoldOuts.Count; i++) {
                ...
                if (GUILayout.Button(new GUIContent(itemName + " objects will be combined into approx. " + (Mathf.CeilToInt(numberOfVertices / _maximumVertices)) + " meshes with a total of " + numberOfVertices + " vertices" + (_infoFoldOuts [i] ? string.Empty : " (Hidden)")), ReorderableListGUI.defaultTitleStyle, GUILayout.Height(20f))) {
                    _infoFoldOuts [i] = !_infoFoldOuts [i];
                }
        ...
        private void UpdateFoldoutsCount() {
            if (_isAtlasingTextures) {
                if (_infoFoldOuts.Count != _dataTreeWithAtlasing.Keys.Count) {
                    if (_infoFoldOuts.Count > _dataTreeWithAtlasing.Keys.Count) {
                        _infoFoldOuts.RemoveRange(_dataTreeWithAtlasing.Keys.Count - 1, _infoFoldOuts.Count - _dataTreeWithAtlasing.Keys.Count);
                ...
                if (_infoFoldOuts.Count != _dataTreeNoAtlasing.Keys.Count) {
                    if (_infoFoldOuts.Count > _dataTreeNoAtlasing.Keys.Count) {
                        _infoFoldOuts.RemoveRange(_dataTreeNoAtlasing.Keys.Count - 1, _infoFoldOuts.Count - _dataTreeNoAtlasing.Keys.Count);
        ...
        private void ShowObjectsList() {
            ...
            if (_isShowingObjectsList) {
                ...
                if (EditorGUI.EndChangeCheck()) {
                    ...
                    IEnumerable<Material> uniqueMaterials = _combinedObjects.Select(currentObject => currentObject).
                                                                            Where(currentObject => currentObject != null).
                                                                            SelectMany(currentObject => currentObject.GetComponentsInChildren<Renderer>().Select(currentRenderer => currentRenderer.sharedMaterial)).
                                                                            Distinct();
        ...
        void ReimportNonReadonlyTextures(IEnumerable<Material> allMaterials) {
            IEnumerable<Texture> allTextures = allMaterials.SelectMany(X => GetAllTextures(X)).Distinct();
        ...
        private Material ExportMaterial(Material exportedMaterial, string materialName) {
            AssetDatabase.CreateAsset(exportedMaterial, _exportPath + materialName + ".mat");
